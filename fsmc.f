...
c     RANDOM NODE SELECTON
      i = INT(genrand_real2()*N) + 1

c     ENERGY DIFFERENDCE
      SUM = 0
      DO k=1,size(NBR(i)%C)
            SUM = SUM + JJ(i)%C(k)*S(NBR(i)%C(k))
      END DO
      DE = 2*S(i)*SUM

c     COMPUTE ACCEPTANCE
...
c     INITIAL RANDOM SPIN CONFIGURATION
      ...
      OPEN(UNIT=2,FILE="data_2.dat")
1     FORMAT(I10,1X,F13.10)

c     MONTE-CARLO SIMULATION
      DO IMC=1,MCTOT
            DO IPAS=1,N
                  CALL METROPOLIS(N,S,VALID,TEMP,DE,NBR,JJ)
                  IF (VALID) THEN
                        ENE = ENE + DE
                  END IF
            END DO
      ...
      call system('gnuplot figure_3.gnu')
