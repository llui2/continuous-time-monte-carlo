...
c     MULTI ARRAY TYPE
      TYPE :: MULTI_ARRAY
      INTEGER,ALLOCATABLE :: C(:) !COLUMN INDEX
...
c-----------------------------------------------------------------------
      ...
      DO j=1,L
            DO i=1,L
                  ...
                  RIGHT = PBC(1,i)+L*(j-1)
                  LEFT = PBC(0,i)+L*(j-1)
                  CALL ADD_ELEMENT(NBR(iSPIN)%C,UP)
                  CALL ADD_ELEMENT(JJ(iSPIN)%C,1)
                  CALL ADD_ELEMENT(NBR(iSPIN)%C,DOWN)
                  CALL ADD_ELEMENT(JJ(iSPIN)%C,1)
                  CALL ADD_ELEMENT(NBR(iSPIN)%C,RIGHT)
                  CALL ADD_ELEMENT(JJ(iSPIN)%C,1)
                  CALL ADD_ELEMENT(NBR(iSPIN)%C,LEFT)
                  CALL ADD_ELEMENT(JJ(iSPIN)%C,1)
                  iSPIN = iSPIN+1
            END DO
      END DO
...
c-----------------------------------------------------------------------
c     PROBABILITY STATES 
c-----------------------------------------------------------------------
      SUBROUTINE PROB_STATES(N,TEMP,PL)
      ...
      E =(/-8.D0,8.D0,-4.D0,4.D0,0.D0/)
      DO i=1,5
            PL(i) = min(1.D0,EXP(-E(i)/TEMP))/N
      END DO
...
c     ENEGY STATES 
c-----------------------------------------------------------------------
      SUBROUTINE STATES(N,S,NBR,JJ,STATE)
      ...
      TYPE(MULTI_ARRAY),ALLOCATABLE :: NBR(:)
      TYPE(MULTI_ARRAY),ALLOCATABLE :: JJ(:)
      TYPE(MULTI_ARRAY),ALLOCATABLE :: STATE(:)
      ...
      ALLOCATE(STATE(5))
      DO i=1,5
            ALLOCATE(STATE(i)%C(0))
      END DO
      DO i=1,N
c           CALCULATE STATE
            SUM = 0
            DO k=1,size(NBR(i)%C)
                  SUM = SUM + S(NBR(i)%C(k))*JJ(i)%C(k)
            END DO
            DE = 2*S(i)*SUM
c           GET STATE LABEL
            ST = 1
            DO WHILE (E(ST).NE.DE)
                  ST = ST+1
            END DO
c           ADD TO CORRESPONDING STATE
            CALL ADD_ELEMENT(STATE(ST)%C,i)
      END DO
...
c-----------------------------------------------------------------------
c     CONTINUOUS TIME MONTE-CARLO STEP
c-----------------------------------------------------------------------
      SUBROUTINE CTMC_STEP(N,S,STATE,PL,TIME)
      ...
      INTEGER i, l, k
      REAL*8 Q
      REAL*8 genrand_real1 !RANDOM U[0,1]
      REAL*8 genrand_real2 !RANDOM U[0,1)
      INTEGER DELTA_TIME
      INTEGER,ALLOCATABLE :: DISTRIB(:)

      ALLOCATE(DISTRIB(0))
c     CREATE THE g_l*P_l DISTRIBUTION
      DO l=1,5
            DO k=1,int(SIZE(STATE(l)%C)*PL(l)*100)
                  CALL ADD_ELEMENT(DISTRIB,l)
            END DO
      END DO
      
c     CALCULATE TRANSITION PROBABILITY
      Q = 0
      DO i=1,5
            Q = Q + SIZE(STATE(i)%C)*PL(i)
      END DO
c     CALCULATE ΔTIME
      DELTA_TIME = INT(ABS(LOG(genrand_real1())/LOG(1-Q)))+1
c     UPDATE TIME
      TIME = TIME + DELTA_TIME
      IF (TIME.LT.0) TIME = 2147483647 !INFINITY
c     SELECT ENERGY STATE
      l = DISTRIB(INT(genrand_real2()*SIZE(DISTRIB))+1)
c     SELECT SPIN WITH ENERGY STATE l
      IF (l.NE.0) THEN
            k = INT(genrand_real2()*SIZE(STATE(l)%C))+1
            k = STATE(l)%C(k)
c           FLIP SPIN
            S(k) = -S(k)
      END IF
...
c-----------------------------------------------------------------------
      ...
      DO i=1,N
            DO k=1,SIZE(NBR(i)%C)
                  ENE = ENE - S(i)*S(NBR(i)%C(k))*JJ(i)%C(k)
            END DO
      END DO
      ENERG =  ENE/2
...
c-----------------------------------------------------------------------
c     MAGNETIZATION FUNCTION 
c-----------------------------------------------------------------------
      REAL*8 FUNCTION MAGNE(N,S)
...
c     MONTE-CARLO
      INTEGER MCTOT, MCINI, MCS
      PARAMETER (MCTOT = 1E4) !TOTAL MONTE-CARLO STEPS
      PARAMETER (MCINI = 0) !EQUILIBRIUM TIME
      PARAMETER (MCS = 1) !CONFIGURATION SAVING INTERVAL
      INTEGER IMC

      INTEGER SEED
      PARAMETER (SEED = 1234) !RANDOM NUMBERS SEED
      INTEGER iSEED, NSEED
      PARAMETER (NSEED = 10)

      REAL*8 genrand_real2 !RANDOM U[0,1)

c     CONTINUOUS MONTE-CARLO
      REAL*8 PL(1:5)
      TYPE(MULTI_ARRAY),ALLOCATABLE:: STATE(:)
      INTEGER TIME, PREVIOUS_TIME
...
c     DEFINE GRAPH STRUCTURE
      ...
      CALL init_genrand(SEED+iSEED)

c     INITIAL RANDOM SPIN CONFIGURATION
      DO i=1,N
            S(i) = INT(2*MOD(INT(2*genrand_real2()),2)-1)
      END DO

c     CALCULATE PROBABILITIES
      CALL PROB_STATES(N,TEMP,PL)

      OPEN(UNIT=3,FILE="data_3.dat")
1     FORMAT(I10,1X,F13.10)

c     MONTE-CARLO SIMULATION
      DO IMC=1,MCTOT

            IF ((IMC.GT.MCINI).AND.(MCS*(IMC/MCS).EQ.IMC)) THEN
                  WRITE(3,1) TIME, MAGNE(N,S)
            END IF

c           CALCULATE SPINS STATES
            CALL STATES(N,S,NBR,JJ,STATE)

            PREVIOUS_TIME = TIME
c           FLIP SPIN AND UPDATE TIME
            CALL CTMC_STEP(N,S,STATE,PL,TIME)
            DEALLOCATE(STATE)

            IF (TIME.EQ.2147483647) THEN
      ...
      CALL SAVE_CONFIG(L,S)
      write(STR1,'(i2)') L
      call system('gnuplot -e L='//STR1//' figure_1.gnu')

      call system('gnuplot figure_2.gnu')
      call system('gnuplot figure_4.gnu')
